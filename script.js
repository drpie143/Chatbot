document.addEventListener("DOMContentLoaded", () => {
  ...
  fetch("./toan_bo_du_lieu_final.json")
    ...
    .catch((error) => {
      ...
      alert(
        "Không thể tải cơ sở dữ liệu thủ tục. Chức năng tìm kiếm sẽ không hoạt động."
      );
    });
  ...
  getEl("home-link").addEventListener("click", (e) => {
    e.preventDefault();
  ...
  chatForm.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      chatMessagesContainer.removeChild(thinkingIndicator);
      appendMessage(data.answer, "bot");
    } catch (error) {
      console.error("Đã có lỗi xảy ra:", error);
      chatMessagesContainer.removeChild(thinkingIndicator);
      appendMessage(
        "Xin lỗi, đã có lỗi xảy ra khi kết nối đến máy chủ. Vui lòng thử lại sau.",
        "bot"
      );
    }
  });
